...
task("reserve_ratio", "Returns the reserve ratio for the stabilizer node")
  .setAction(async ({ }, { ethers, network }) => {
    ...
    const daiReserves = await dai.balanceOf(artifacts.maltPair.address);
    const maltReserves = await malt.balanceOf(artifacts.maltPair.address);

    console.log(`LE dai balance: ${utils.commify(utils.formatEther(daiBalance))}`);
    console.log(`Pool DAI Reserves: ${utils.commify(utils.formatEther(daiReserves))}`);
    console.log(`Pool Malt Reserves: ${utils.commify(utils.formatEther(maltReserves))}`);
