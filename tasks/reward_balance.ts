...
task("reward_balance", "Returns how much an account has been allocated")
  ...
  .setAction(async ({ account }, { ethers, network }) => {
    ...
    const bal = await miningService.netRewardBalance(account, 0);
    const rewardMineAllocatedReward = await erc20VestedMine.netRewardBalance(account);
    const auctionPoolAllocatedReward = await auctionPool.netRewardBalance(account);
    ...
    const earned = await miningService.earned(account, 0);
