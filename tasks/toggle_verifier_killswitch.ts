...
task("toggle_verifier_killswitch", "Toggle the state of transfer verifier killswitch")
  .setAction(async ({}, { ethers, network }) => {
    ...
    const tx = await verifier.toggleKillswitch({
      gasPrice: utils.parseUnits('110', 'gwei'),
    });
    await tx.wait();
