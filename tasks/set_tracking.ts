...
task("set_tracking", "Set the cut of profits on stabilize calls")
  .setAction(async ({}, { ethers, network }) => {
    ...
    const rewardThrottle = await ethers.getContractAt("UniV2PoolKeeper", artifacts.keeper.address);

    // dao, lp, caller, auctionPool, swingTrader
    const tx = await rewardThrottle.setUpkeepTracking(false);
    await tx.wait();
  });
