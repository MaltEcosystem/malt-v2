...
task("auction_info", "Checks infomation about auctions")
  .setAction(async ({}, { ethers, network }) => {
    ...
    const auctionBalance = await rewardToken.balanceOf(auction.address);
    ...
    console.log(`Auction reward token balance: ${utils.commify(utils.formatEther(auctionBalance))}`);
  });
