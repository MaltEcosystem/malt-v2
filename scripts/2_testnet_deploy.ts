...
async function deploy() {
  ...
  try {
    ...
    const factory = await factoryContract.deploy(constants.AddressZero, txConfig);
    const weth = await wethContract.deploy(txConfig);
    const router = await routerContract.deploy(factory.address, weth.address, txConfig);
    ...
    const felipe = "0x45D3f6fA3A7eFC3a6007265359E618A5b997DCb7";
    const krn = "0x8DF60274Ee7db022c8911d87F9F5311C21b9B5aD";
    const maltSetup = await malt.initialSupplyControlSetup(
      [dao.address, escapeHatch.address, stabilizerNode.address, signerAddress, felipe, krn],
      [liquidityExtension.address],
      txConfig
    );
