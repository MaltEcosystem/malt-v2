...
export async function deterministicDeploy(contract: any, constructorTypes: string[] = [], constructorArgs: any[] = [], create2Address: string) {
  if (!create2Address) {
  ...
  let deployment;
  if (!isDeployed) {
    const create2Deployer = new ethers.Contract(create2Address, ["function deploy(bytes memory code, uint256 salt) external"], signer);
  ...
  return {
    contract: new ethers.Contract(computedAddr, contract.interface.format(), signer),
    deployment,
  }
}
