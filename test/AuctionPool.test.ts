...
describe("AuctionPool", function() {
  ...
  beforeEach(async function() {
    ...
    await mockDataLab.mock.lastMaltPrice.returns(utils.parseEther('1'), 0);
    await mockDataLab.mock.trackPool.returns();
    await mockBonding.mock.balanceOfBonded.returns(0);
    ...
    auctionPool = (await AuctionPoolFactory.deploy(
      ownerAddress,
      adminAddress,
      0
    )) as AuctionPool;
    await auctionPool.deployed();
    ...
    await auctionPool.setupContracts(
      auction.address,
  ...
  it("Returns balanceOfBonded from the bonding contract", async function() {
    ...
    const amountBonded = utils.parseEther('2357');
    await mockBonding.mock.balanceOfBonded.withArgs(0, userAddress).returns(amountBonded);
  ...
  it("Handles unbonding with no bonded balance", async function() {
    ...
    // Mock
    await mockBonding.mock.totalBonded.returns(0);
    await mockBonding.mock.balanceOfBonded.withArgs(0, userAddress).returns(0);
