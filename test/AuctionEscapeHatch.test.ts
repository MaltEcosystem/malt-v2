...
import { hardhatSnapshot, hardhatRevert, increaseTime, setNextBlockTime } from "./helpers";
...
describe("AuctionEscapeHatch", function() {
  ...
  beforeEach(async function() {
    ...
    mockAuction = ((await deployMockContract(owner, [
      "function getAuctionCore(uint256) returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, bool)",
      "function getAuctionParticipationForAccount(address, uint256) returns (uint256, uint256, uint256, uint256)",
      "function accountExit(address, uint256, uint256)"
    ])) as any) as Auction;
    mockUniswapHandler = ((await deployMockContract(owner, [
      "function maltMarketPrice() returns (uint256, uint256)",
      "function sellMalt(uint256, uint256) returns (uint256)",
    ...
    auctionEscapeHatch = (await AuctionEscapeHatchFactory.deploy(
      ownerAddress,
      adminAddress,
      mockAuction.address,
    ...
    await malt.initialSupplyControlSetup([ownerAddress, auctionEscapeHatch.address], []);
  ...
  it("Has correct initial conditions", async function() {
    ...
    expect(await auctionEscapeHatch.maxEarlyExitBps()).to.equal(2000);
  });
  ...
  it("Reverts when calling exit early on an active auction", async function() {
    ...
    await mockAuction.mock.getAuctionCore.withArgs(0).returns(
      0, purchaseAmount, maltPurchased, 0, 0, pegPrice, 0, 0, 0, isActive);
    await mockAuction.mock.getAuctionParticipationForAccount.withArgs(userAddress, 0).returns(
      purchaseAmount, 0, maltPurchased, 0
    );

    const expectedReturn = await auctionEscapeHatch.earlyExitReturn(userAddress, 0, purchaseAmount);
    expect(expectedReturn[0]).to.equal(0);
    await expect(auctionEscapeHatch.connect(user1).exitEarly(0, purchaseAmount, 0)).to.be.revertedWith("ExitEarly: Insufficient output");
  });
  ...
  it("Reverts when calling exit early with 0 amount", async function() {
    ...
    const expectedReturn = await auctionEscapeHatch.earlyExitReturn(userAddress, 0, purchaseAmount);
    expect(expectedReturn[0]).to.equal(purchaseAmount);
    await expect(auctionEscapeHatch.connect(user1).exitEarly(0, 0, 0)).to.be.revertedWith("ExitEarly: Insufficient output");
  });
  ...
  it("Returns correct amount when exiting early above the entry", async function() {
    ...
    await mockUniswapHandler.mock.maltMarketPrice.returns(pegPrice, 18);
    await mockAuction.mock.accountExit.withArgs(userAddress, 0, purchaseAmount).returns();
    ...
    expect(expectedReturn[0]).to.equal(amountRewarded);
  ...
  it("Returns nothing when calling exit early after all reward has been redeemed", async function() {
    ...
    await mockAuction.mock.getAuctionParticipationForAccount.withArgs(userAddress, 0).returns(
      purchaseAmount, purchaseAmount, maltPurchased, 0
    );
  ...
  it("Returns 0 profit when exiting early at the end of the auction despite price being well above entry", async function() {
    ...
    const maltPurchased = utils.parseEther('2000'); // implies price of $0.5
    const isActive = false;
    const endingTime = currentTime - 10;
    // endingTime is set to 0 to ensure we are after cooloff period
    // price is also at peg but the profit won't be realisable yet
    await mockAuction.mock.getAuctionCore.withArgs(0).returns(
      0, purchaseAmount, maltPurchased, 0, 0, pegPrice, 0, endingTime, 0, isActive);
    await mockAuction.mock.getAuctionParticipationForAccount.withArgs(userAddress, 0).returns(
      purchaseAmount, 0, maltPurchased, 0
    );
    await mockUniswapHandler.mock.maltMarketPrice.returns(pegPrice, 18);
    ...
    let expectedReturn = await auctionEscapeHatch.earlyExitReturn(userAddress, 0, purchaseAmount);
    // Nothing returned immediately
    expect(expectedReturn[0]).to.equal(0);

    // Fast forward to 10% of 24 hours (the cooloff period)
    await increaseTime(60 * 60 * 2.4);

    expectedReturn = await auctionEscapeHatch.earlyExitReturn(userAddress, 0, purchaseAmount);
    // Because we are right at the end of the auction the max profit is close to 0 despite price being double entry
    expect(expectedReturn[0]).to.withinPercent(utils.parseEther('1020'), 0.0001); // within 0.01%
    ...
    expect(auctionExitedEarly).to.equal(purchaseAmount);
    expect(auctionExitReturn).to.equal(purchaseAmount);
    expect(auctionMaltUsed).to.be.withinPercent(utils.parseEther('1020'), 0.0001);

    const [accountExitedEarly, accountExitReturn, accountMaltUsed] = await auctionEscapeHatch.accountAuctionExits(userAddress, 0);
    expect(accountExitedEarly).to.equal(purchaseAmount);
    expect(accountExitReturn).to.equal(purchaseAmount);
    expect(accountMaltUsed).to.be.withinPercent(utils.parseEther('1020'), 0.0001);
  });
  ...
  it("Returns half max profit when exiting early half way through cooloff period", async function() {
    ...
    const endingTime = currentTime - (cooloffPeriod / 2); // puts us 1/2 way into cooloff period
    // endingTime is set to 0 to ensure we are after cooloff period
    // price is also at peg but the profit won't be realisable yet
    await mockAuction.mock.getAuctionCore.withArgs(0).returns(
      0, purchaseAmount, maltPurchased, 0, 0, pegPrice, 0, endingTime, 0, isActive);
    ...
    const expectedReturn = await auctionEscapeHatch.earlyExitReturn(userAddress, 0, purchaseAmount);
    expect(expectedReturn[0]).to.equal(desiredReturn);
    ...
    expect(auctionMaltUsed).to.be.withinPercent(desiredReturn, 0.0001);

    const [accountExitedEarly, accountExitReturn, accountMaltUsed] = await auctionEscapeHatch.accountAuctionExits(userAddress, 0);
    expect(accountExitedEarly).to.equal(purchaseAmount);
    expect(accountExitReturn).to.equal(desiredReturn);
    expect(accountMaltUsed).to.be.withinPercent(desiredReturn, 0.0001);
  });
});
