...
describe("Stabilizer Node", function() {
  ...
  beforeEach(async function() {
    ...
    stabilizerNode = (await StabilizerNodeFactory.deploy(
      ownerAddress,
      adminAddress,
      malt.address,
      dai.address,
    ...
    await mockDataLab.mock.trackPool.returns();
    ...
    await malt.initialSupplyControlSetup(
      [ownerAddress, stabilizerNode.address],
      []
    );
    await dai.initialSupplyControlSetup(
      [ownerAddress],
      []
    );
    ...
    await uniswapHandler.connect(admin).grantRole(SELLER_ROLE, stabilizerNode.address);
  });
  ...
  it("Has correct initial conditions", async function() {
    ...
    expect(await stabilizerNode.upperStabilityThresholdBps()).to.equal(100);
    expect(await stabilizerNode.lowerStabilityThresholdBps()).to.equal(100);
    expect(await stabilizerNode.maxContributionBps()).to.equal(7000);
    expect(await stabilizerNode.priceAveragePeriod()).to.equal(5 * 60); // 5 minutes
    expect(await stabilizerNode.fastAveragePeriod()).to.equal(30); // 30 seconds
    expect(await stabilizerNode.expansionDampingFactor()).to.equal(1);
    expect(await stabilizerNode.defaultIncentive()).to.equal(100);
    expect(await stabilizerNode.daoRewardCutBps()).to.equal(0);
    expect(await stabilizerNode.lpRewardCutBps()).to.equal(4170);
    expect(await stabilizerNode.auctionPoolRewardCutBps()).to.equal(1130);
    expect(await stabilizerNode.swingTraderRewardCutBps()).to.equal(4170);
    expect(await stabilizerNode.treasuryRewardCutBps()).to.equal(500);
    expect(await stabilizerNode.callerRewardCutBps()).to.equal(30);
    ...
    expect(await stabilizerNode.overrideDistanceBps()).to.equal(200);
  });
  ...
  it("Handles calling stabilize when price is at peg", async function() {
    ...
    await mockAuction.mock.auctionExists.withArgs(0).returns(false);
  ...
  it("Starts an auction when price is below peg", async function() {
    ...
    await uniswapHandler.addLiquidity(utils.parseEther('1000'), utils.parseEther('900'), 5000);
  ...
  it("Distributes supply when price is above peg", async function() {
    ...
    await mockLiquidityExtension.mock.collateralDeficit.returns(0, 18);
    // Don't use it all
    ...
    await uniswapHandler.addLiquidity(utils.parseEther('1000'), utils.parseEther('1100'), 5000);
  ...
  it("Disallows calling stabilize before end of stabilization backoff window", async function() {
    ...
    await mockDataLab.mock.maltPriceAverage.returns(utils.parseEther('1.001'));
  ...
  it("It only allows admins to update reward cut", async function() {
    ...
    expect(await stabilizerNode.daoRewardCutBps()).to.equal(dao);
    expect(await stabilizerNode.lpRewardCutBps()).to.equal(lp);
    expect(await stabilizerNode.callerRewardCutBps()).to.equal(caller);
    expect(await stabilizerNode.treasuryRewardCutBps()).to.equal(treasuryCut);
    expect(await stabilizerNode.auctionPoolRewardCutBps()).to.equal(auctionPoolCut);
    expect(await stabilizerNode.swingTraderRewardCutBps()).to.equal(swingTraderCut);
  ...
  it("It only allows admins to update stability thresholds", async function() {
    ...
    expect(await stabilizerNode.upperStabilityThresholdBps()).to.equal(newValue);
    expect(await stabilizerNode.lowerStabilityThresholdBps()).to.equal(newValueTwo);

    // Default signer has the Timelock role
    await stabilizerNode.setStabilityThresholds(422, 88);
    expect(await stabilizerNode.upperStabilityThresholdBps()).to.equal(422);
    expect(await stabilizerNode.lowerStabilityThresholdBps()).to.equal(88);
  });
  ...
  it("It only allows admins to update override distance", async function() {
    ...
    expect(await stabilizerNode.overrideDistanceBps()).to.equal(newValue);

    // Default signer has the Timelock role
    await stabilizerNode.setOverrideDistance(42);
    expect(await stabilizerNode.overrideDistanceBps()).to.equal(42);
  });
});
