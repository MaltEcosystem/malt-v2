...
describe("Reward Throttle", function() {
  ...
  beforeEach(async function() {
    ...
    await mockBonding.mock.poolAllocations.returns(
      [0],
      [utils.parseEther('1')],
    ...
    rewardOverflow = (await RewardOverflowFactory.deploy(
      ownerAddress,
      adminAddress,
    ...
    rewardThrottle = (await RewardThrottleFactory.deploy(
      ownerAddress,
      adminAddress,
      mockDAO.address,
    ...
    await rewardThrottle.setDesiredRunway(2629800 * 24); // 2 years

    await rewardOverflow.setupContracts(
  ...
  it("Has correct initial conditions", async function() {
    ...
    expect(await rewardThrottle.throttleBps()).to.equal(2000);
  ...
  it("Sends all rewards to distributor in epoch 0", async function() {
    ...
    expect(throttleAmount).to.equal(2000);
  });
  ...
  it("Handles rewards across epoch boundaries", async function() {
    ...
    expect(throttleAmountTwo).to.equal(2000);
    ...
    expect(throttleAmountThree).to.equal(2000);
  });
  ...
  it("If an epoch underflows it will pull all it needs from the overflow pool", async function() {
    ...
    expect(finalThrottleAmount).to.equal(2000);
  ...
  it("It only allows admins to update throttle amount", async function() {
    ...
    await expect(rewardThrottle.connect(user).setThrottleBps(10)).to.be.reverted;
    await expect(rewardThrottle.connect(auctionContract).setThrottleBps(10)).to.be.reverted;

    const newThrottle = 356;
    await rewardThrottle.connect(admin).setThrottleBps(newThrottle);
    expect(await rewardThrottle.throttleBps()).to.equal(newThrottle);

    // Default signer has the Timelock role
    await rewardThrottle.setThrottleBps(422);
    expect(await rewardThrottle.throttleBps()).to.equal(422);
  });
  ...
  it("Correctly distributes based on poolAllocations", async function() {
    const distributorOne = ((await deployMockContract(owner, ["function declareReward(uint256)"])) as any) as RewardDistributor;
    const distributorTwo = ((await deployMockContract(owner, ["function declareReward(uint256)"])) as any) as RewardDistributor;

    const distributorOneShare = 0.2342513523;
    const distributorTwoShare = 1 - distributorOneShare;

    await mockBonding.mock.poolAllocations.returns(
      [0, 1],
      [utils.parseEther(distributorOneShare.toString()), utils.parseEther(distributorTwoShare.toString())],
      [distributorOne.address, distributorTwo.address]
    );

    const distributorOneBalance = await dai.balanceOf(distributorOne.address);
    const distributorTwoBalance = await dai.balanceOf(distributorTwo.address);

    expect(distributorOneBalance).to.equal(0);
    expect(distributorTwoBalance).to.equal(0);

    const rewardValue = 2000;
    const amountDai = utils.parseEther(rewardValue.toString());

    await mockDAO.mock.epoch.returns(0);
    await mockBonding.mock.averageBondedValue.withArgs(0).returns(amountDai);
    await distributorOne.mock.declareReward.returns();
    await distributorTwo.mock.declareReward.returns();

    await dai.mint(rewardThrottle.address, amountDai);
    await rewardThrottle.handleReward();

    const finalDistributorOneBalance = await dai.balanceOf(distributorOne.address);
    expect(finalDistributorOneBalance).to.be.withinPercent(utils.parseEther((rewardValue * distributorOneShare).toString()), 0.001);
    const finalDistributorTwoBalance = await dai.balanceOf(distributorTwo.address);
    expect(finalDistributorTwoBalance).to.be.withinPercent(utils.parseEther((rewardValue * distributorTwoShare).toString()), 0.001);

    const finalThrottleBalance = await dai.balanceOf(rewardThrottle.address);
    expect(finalThrottleBalance).to.equal(0);
  });

  it("Correctly handles discrepency in poolAllocations", async function() {
    const distributorOne = ((await deployMockContract(owner, ["function declareReward(uint256)"])) as any) as RewardDistributor;
    const distributorTwo = ((await deployMockContract(owner, ["function declareReward(uint256)"])) as any) as RewardDistributor;

    const distributorOneShare = 0.2342513523;
    // Share two is 10% larger than is allowed
    const distributorTwoShare = 1.1 - distributorOneShare;
    ...
    const shareOne = rewardValue * distributorOneShare;
    const finalDistributorOneBalance = await dai.balanceOf(distributorOne.address);
    expect(finalDistributorOneBalance).to.be.withinPercent(utils.parseEther((shareOne).toString()), 0.001);
    const finalDistributorTwoBalance = await dai.balanceOf(distributorTwo.address);
    expect(finalDistributorTwoBalance).to.be.withinPercent(utils.parseEther((rewardValue - shareOne).toString()), 0.001);
