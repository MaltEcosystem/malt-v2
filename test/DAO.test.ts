...
describe("DAO", function() {
  ...
  beforeEach(async function() {
    ...
    dao = (await MaltDAOFactory.deploy(
      ownerAddress,
      adminAddress,
      epochLength,
      genesisTime,
    ...
    await malt.initialSupplyControlSetup([ownerAddress, dao.address], []);

    expect(await malt.balanceOf(offeringAddress)).to.equal(0);

    await dao.setMaltToken(malt.address, offeringMint, offeringAddress);
