...
describe("ERC20 Vested Mine", function() {
  ...
  beforeEach(async function() {
    ...
    mockLpToken = ((await deployMockContract(owner, [
    ...
    mockDistributor = ((await deployMockContract(owner, [
      ...
      "function getFocalUnvestedBps(uint256) returns (uint256)",
      "function focalID() returns (uint256)"
    ])) as any) as RewardDistributor;
    mockBonding = ((await deployMockContract(owner, [
      "function totalBonded() returns (uint256)",
      "function balanceOfBonded(uint256, address) returns(uint256)",
      "function forfeit(uint256)",
      "function stakeTokenDecimals() returns(uint256)"
    ])) as any) as Bonding;

    await mockBonding.mock.stakeTokenDecimals.returns(18);
    await mockDistributor.mock.focalID.returns(1);
    await mockDistributor.mock.getFocalUnvestedBps.returns(10000);

    // Deploy the Auction
    const ERC20VestedMineFactory = await ethers.getContractFactory("ERC20VestedMine");

    lpMine = (await ERC20VestedMineFactory.deploy(
      ownerAddress,
      adminAddress,
    ...
    await mockBonding.mock.balanceOfBonded.withArgs(0, userOneAddress).returns(0);
    await mockBonding.mock.balanceOfBonded.withArgs(0, userTwoAddress).returns(0);
  });
  ...
  it("Correctly returns balance of bonded from bonding contract", async function() {
    ...
    const bondedAmount = utils.parseEther('23407');
    await mockBonding.mock.balanceOfBonded.withArgs(0, userAddress).returns(bondedAmount);
  ...
  it("Doesn't change totalReleased when reward tokens are sent to the mine without declaring them", async function() {
    ...
    expect(finalTotalReleased).to.equal(0);
  });

  it("Disallows non privileged addresses from declaring rewards", async function() {
    const [user] = accounts;
    const totalReleased = await lpMine.totalReleasedReward();
    expect(totalReleased).to.equal(0);
    ...
    const releasedAmount = utils.parseEther('8234');
    await dai.mint(lpMine.address, releasedAmount);
    await expect(lpMine.connect(user).releaseReward(releasedAmount)).to.be.reverted;
  ...
  it("Correctly returns total released reward", async function() {
    ...
    const releasedAmount = utils.parseEther('8234');
    await dai.mint(lpMine.address, releasedAmount);
    await lpMine.releaseReward(releasedAmount);
  ...
  it("Correctly attributes all reward to the only bonded user", async function() {
    ...
    await mockBonding.mock.balanceOfBonded.withArgs(0, userAddress).returns(bondAmount);
    await lpMine.declareReward(reward);
    ...
    await lpMine.releaseReward(reward.div(2));
    await mockDistributor.mock.getFocalUnvestedBps.returns(5000);
    ...
    await dai.mint(lpMine.address, reward.div(2));
    await lpMine.releaseReward(reward.div(2));
    await mockDistributor.mock.getFocalUnvestedBps.returns(0);
  ...
  it("Correctly attributes all reward to the only bonded user when another user bonds afterwards", async function() {
    ...
    await mockDistributor.mock.totalDeclaredReward.returns(reward);
    await mockBonding.mock.totalBonded.returns(bondOneAmount);
    await mockBonding.mock.balanceOfBonded.withArgs(0, userOneAddress).returns(bondOneAmount);
    await lpMine.declareReward(reward);

    await lpMine.connect(miningService).onBond(userTwoAddress, bondTwoAmount);

    await mockBonding.mock.totalBonded.returns(bondOneAmount.add(bondTwoAmount));
    await mockBonding.mock.balanceOfBonded.withArgs(0, userTwoAddress).returns(bondTwoAmount);
  ...
  it("Allows user to withdraw their earned rewards after they have all vested", async function() {
    ...
    await lpMine.releaseReward(reward);
    await mockDistributor.mock.getFocalUnvestedBps.returns(0);
  ...
  it("Allows user to withdraw the full amount from their partially vested rewards", async function() {
    ...
    await lpMine.releaseReward(vestedAmount);
    await mockDistributor.mock.getFocalUnvestedBps.returns(5000);
  ...
  it("Handles second user bonding and unbonding", async function() {
    ...
    await mockBonding.mock.balanceOfBonded.withArgs(0, userOneAddress).returns(bondAmount);
    await mockBonding.mock.totalBonded.returns(bondAmount);
    await mockDistributor.mock.totalDeclaredReward.returns(reward);
    await lpMine.declareReward(reward);
    ...
    const newReward = utils.parseEther('200');
    reward = reward.add(newReward);
    await lpMine.connect(miningService).onBond(userTwoAddress, bondAmount);
    await mockBonding.mock.balanceOfBonded.withArgs(0, userTwoAddress).returns(bondAmount);
    await mockBonding.mock.totalBonded.returns(bondAmount.mul(2));
    await mockDistributor.mock.totalDeclaredReward.returns(reward);
    await lpMine.declareReward(newReward);
    ...
    // USER 2 BOND AGAIN
    await lpMine.connect(miningService).onBond(userTwoAddress, bondAmount);
    await mockBonding.mock.balanceOfBonded.withArgs(0, userTwoAddress).returns(bondAmount.mul(2));
    ...
    expect(userOneReward).to.equal(desiredUserOneReward);
    expect(userTwoReward).to.equal(desiredUserTwoReward);
    expect(await lpMine.earned(userOneAddress)).to.equal(0);
    expect(await lpMine.earned(userTwoAddress)).to.equal(0);
  ...
  it("Additional stakers don't drop already vested rewards for existing users", async function() {
    const [user1, user2] = accounts;
    const userOneAddress = await user1.getAddress();
    const userTwoAddress = await user2.getAddress();
    ...
    let releasedAmount = utils.parseEther('50');
    await dai.mint(lpMine.address, releasedAmount);
    await lpMine.releaseReward(releasedAmount);
    await mockDistributor.mock.getFocalUnvestedBps.returns(5000);
    ...
    let userOneReward = await lpMine.balanceOfRewards(userOneAddress);
    let userTwoReward = await lpMine.balanceOfRewards(userTwoAddress);
    expect(userOneReward).to.equal(utils.parseEther('100'));
    expect(userTwoReward).to.equal(0);

    let userOneEarned = await lpMine.earned(userOneAddress);
    let userTwoEarned = await lpMine.earned(userTwoAddress);
    expect(userOneEarned).to.equal(releasedAmount);
    expect(userTwoEarned).to.equal(0);

    // USER 2 BOND
    // Add another 100 in rewards
    const newReward = utils.parseEther('100');
    reward = reward.add(newReward);
    await lpMine.connect(miningService).onBond(userTwoAddress, bondAmount);
    await mockBonding.mock.balanceOfBonded.withArgs(0, userTwoAddress).returns(bondAmount);
    await mockBonding.mock.totalBonded.returns(bondAmount.mul(2));
    ...
    // user 1 gets all 100 of original reward and half of the 100 additional.
    // Therefore of the 200 total reward, user 1 is owed 150 of it
    userOneReward = await lpMine.balanceOfRewards(userOneAddress);
    userTwoReward = await lpMine.balanceOfRewards(userTwoAddress);
    expect(userOneReward).to.equal(utils.parseEther('150'));
    expect(userTwoReward).to.equal(utils.parseEther('50'));

    userOneEarned = await lpMine.earned(userOneAddress);
    userTwoEarned = await lpMine.earned(userTwoAddress);
    expect(userOneEarned).to.equal(releasedAmount);
    expect(userTwoEarned).to.equal(0);
  });

  it("Additional stakers don't change rewards", async function() {
    const [user1, user2] = accounts;
    const userOneAddress = await user1.getAddress();
    const userTwoAddress = await user2.getAddress();
    ...
    const rewardTwo = utils.parseEther('1000');
    let totalReward = reward.add(rewardTwo);
    await mockDistributor.mock.totalDeclaredReward.returns(totalReward);
    await lpMine.declareReward(rewardTwo);

    const thirdUserOneEarned = await lpMine.earned(userOneAddress);
    const thirdUserTwoEarned = await lpMine.earned(userTwoAddress);
    expect(thirdUserOneEarned).to.equal(reward.div(2));
    expect(thirdUserTwoEarned).to.equal(0);
  });

  it("Forfeiting reward doesn't change vesting process", async function() {
    ...
    await mockDistributor.mock.forfeit.withArgs(reward.div(4)).returns();
    await mockDistributor.mock.decrementRewards.withArgs(reward.div(4)).returns();
    await lpMine.connect(miningService).onUnbond(userOneAddress, bondOneAmount.div(2));

    // Releasing another quarter actually completes the vesting as 25% of reward
    // was forfeited by unbonding half of the LP halfway through the vest
    await dai.mint(lpMine.address, reward.div(4));
    await lpMine.releaseReward(reward.div(4));
    await mockDistributor.mock.getFocalUnvestedBps.returns(0);

    const thirdUserOneEarned = await lpMine.earned(userOneAddress);
    expect(thirdUserOneEarned).to.equal(reward.mul(3).div(4));
  });

  it("Correctly handles unvested calculations when user bonds after previous focal has rewards", async function() {
    const [user1, user2] = accounts;
    const userOneAddress = await user1.getAddress();
    const userTwoAddress = await user2.getAddress();
    ...
    await lpMine.releaseReward(releasedAmount);
    await mockDistributor.mock.getFocalUnvestedBps.returns(5000);
    // increment focalID now that half the original focal has passed
    await mockDistributor.mock.focalID.returns(2);
    ...
    const unvested = await lpMine.accountUnvested(userTwoAddress);

    // user has just bonded so has no rewards. So 0 should be unvested
    expect(unvested).to.equal(0);

    const userOneUnvested = await lpMine.accountUnvested(userOneAddress);
    expect(userOneUnvested).to.equal(reward.sub(releasedAmount));
  });
});
