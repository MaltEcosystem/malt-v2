...
describe("Reward Overflow Pool", function() {
  ...
  beforeEach(async function() {
    ...
    rewardOverflow = (await RewardOverflowPoolFactory.deploy(
      ownerAddress,
      adminAddress,
  ...
  it("Has correct initial conditions", async function() {
    ...
    expect(await rewardOverflow.maxFulfillmentBps()).to.equal(5000);
  ...
  it("Allows admins to set new max fulfillment", async function() {
    ...
    expect(await rewardOverflow.maxFulfillmentBps()).to.equal(newMaxFulfillment);

    const newReserveRatio2 = 40;
    await rewardOverflow.setMaxFulfillment(newReserveRatio2);
    expect(await rewardOverflow.maxFulfillmentBps()).to.equal(newReserveRatio2);
  });
