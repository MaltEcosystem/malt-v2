...
describe("Swing Trader", function() {
  ...
  beforeEach(async function() {
    ...
    swingTrader = (await SwingTraderFactory.deploy(
      ownerAddress,
      adminAddress,
      dai.address,
      malt.address,
    ...
    await swingTrader.deployed();

    await swingTrader.setProfitGreed(10000);
    ...
    await uniswapHandler.addLiquidity(maltReserves, daiReserves, 5000);

    const BUYER_ROLE = utils.id("BUYER_ROLE");
    await uniswapHandler.connect(admin).grantRole(BUYER_ROLE, swingTrader.address);
    const SELLER_ROLE = utils.id("SELLER_ROLE");
    await uniswapHandler.connect(admin).grantRole(SELLER_ROLE, swingTrader.address);
  ...
  it("Has correct initial conditions", async function() {
    ...
    expect(await swingTrader.lpProfitCutBps()).to.equal(5000);
  });
  ...
  it("It a portion of profits to rewardThrottle", async function() {
    ...
    // This is based on the default lpProfitCutBps of 50%
  ...
  it("It only allows admins to update lpProfitCutBps", async function() {
    expect(await swingTrader.lpProfitCutBps()).to.equal(5000);
    ...
    expect(await swingTrader.lpProfitCutBps()).to.equal(200);

    // Default signer has the Timelock role
    await swingTrader.setLpProfitCut(400);
    expect(await swingTrader.lpProfitCutBps()).to.equal(400);
  });

  it("It only allows values up to 100% for lpProfitCut", async function() {
    await expect(swingTrader.connect(admin).setLpProfitCut(10001)).to.be.reverted;
    await expect(swingTrader.setLpProfitCut(30001)).to.be.reverted;
  });
});
