...
describe("Auction", function() {
  ...
  beforeEach(async function() {
    ...
    mockDataLab = ((await deployMockContract(owner, [
      ...
      "function lastMaltPrice() returns (uint256, uint256)",
      "function trackPool()",
    ...
    mockLiquidityExtension = ((await deployMockContract(owner, [
      ...
      "function reserveRatioAverage(uint256) returns (uint256, uint256)"
    ...
    await mockDataLab.mock.lastMaltPrice.returns(utils.parseEther('1'), 0); // second arg doesn't matter
    await mockAuctionPool.mock.declareReward.returns();
    ...
    await dai.mint(mockLiquidityExtension.address, daiReserves);
  ...
  it("Has correct initial conditions", async function() {
    ...
    expect(await auction.arbTokenReplenishSplitBps()).to.equal(7000);
    expect(await auction.maxAuctionEndBps()).to.equal(9000);
  });
  ...
  it("Disallows non-stabilizerNode from triggering an auction", async function() {
    ...
    // Add mocks
    await mockLiquidityExtension.mock.reserveRatioAverage.returns(utils.parseEther('0.4'), 18);
  ...
  it("Only allows the amender to add an account exit", async function() {
    const pegPrice = utils.parseEther('1');
    const purchaseAmount = utils.parseEther('1000');
    const [user] = accounts;
    const userAddress = await user.getAddress();
    ...
    const [initialUserCommitment, initialUserRedeemed, initialUserMaltPurchased, initialUserExited] = await auction.getAuctionParticipationForAccount(userAddress, 0);
    ...
    expect(initialUserExited).to.equal(0);

    await expect(auction.accountExit(userAddress, 0, userCommitment)).to.be.reverted;

    await auction.connect(amender).accountExit(userAddress, 0, userCommitment);

    const [finalUserCommitment, finalUserRedeemed, finalUserMaltPurchased, finalUserExited] = await auction.getAuctionParticipationForAccount(userAddress, 0);
    expect(finalUserCommitment).to.equal(initialUserCommitment);
    expect(finalUserRedeemed).to.equal(initialUserRedeemed);
    expect(finalUserMaltPurchased).to.equal(initialUserMaltPurchased);
    expect(finalUserExited).to.equal(userCommitment);
    ...
    expect(commitments).to.equal(userCommitment);
    expect(maxCommitments).to.equal(userCommitment);
    ...
    expect(auctions[0]).to.equal(0);
    expect(userCommitments[0]).to.equal(userCommitment);
    expect(awardedTokens[0]).to.be.withinPercent(utils.parseEther('750'));
    expect(redeemedTokens[0]).to.equal(0);
    expect(exitedTokens[0]).to.be.withinPercent(utils.parseEther('750'));
    expect(finalPrice[0]).to.be.withinPercent(utils.parseEther(currentPrice.toString()));
    expect(claimable[0]).to.equal(0);
    expect(finished[0]).to.equal(true);
  ...
  it("It only allows admins to update auction replenish split", async function() {
    ...
    expect(await auction.arbTokenReplenishSplitBps()).to.equal(200);

    // Default signer has the Timelock role
    await auction.setTokenReplenishSplit(400);
    expect(await auction.arbTokenReplenishSplitBps()).to.equal(400);
  });
  ...
  it("It only allows admins to update max auction end", async function() {
    ...
    expect(await auction.maxAuctionEndBps()).to.equal(200);

    // Default signer has the Timelock role
    await auction.setMaxAuctionEnd(400);
    expect(await auction.maxAuctionEndBps()).to.equal(400);
  });
  ...
  it("Correctly returns from hasOngoingAuction", async function() {
    const pegPrice = utils.parseEther('1');
    const purchaseAmount = utils.parseEther('1000');
    const [user] = accounts;
    const userAddress = await user.getAddress();
    ...
    expect(await auction.hasOngoingAuction()).to.equal(false);

    await auction.connect(stabilizerNode).triggerAuction(pegPrice, purchaseAmount);

    expect(await auction.hasOngoingAuction()).to.equal(true);
    ...
    await mockLiquidityExtension.mock.purchaseAndBurn.withArgs(burnBudget).returns(utils.parseEther('500'));
